...
function removeFirstOccurrences(str, value) {
  ...
  const end = str.substring(pos + value.length, str.length);
  return start + end;
}
...
function isPalindrome(str) {
  ...
  const strReversed = reverseString(strCleared);
  return strCleared === strReversed;
}
...
function invertCase(str) {
  ...
  const inverted = str.split('').map(toggleCase);
